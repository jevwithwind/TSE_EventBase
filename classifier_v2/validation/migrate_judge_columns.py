"""
migrate_judge_columns.py -- Add LLM-as-judge columns to the events table.
...
Usage:
    ...
    python migrate_judge_columns.py --db data/tse_eventbase.db --dry-run
"""
...
NEW_COLUMNS = [
    ("judge_event_type", "TEXT"),
    ("judge_event_subtype", "TEXT"),
    ("judge_direction", "TEXT"),
    ("judge_magnitude", "TEXT"),
    ("judge_confidence", "TEXT"),
    ("judge_headline_en", "TEXT"),
    ("judge_summary", "TEXT"),
    ("judge_classified_at", "TEXT"),
    ("judge_failed_at", "TEXT"),
    ("judge_error", "TEXT"),
]
...
def migrate(db_path: str, dry_run: bool = False) -> dict:
    ...
    if not Path(db_path).exists():
        raise RuntimeError(
            ...
            "Check the path -- common gotcha is filename "
            "(e.g., 'tse_eventbase.db' vs 'eventbase.db')."
        )
...
def main():
    p = argparse.ArgumentParser(description="Add LLM-as-judge columns to events table.")
